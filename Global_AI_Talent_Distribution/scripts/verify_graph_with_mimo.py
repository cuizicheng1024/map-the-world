...
def l2_enrich_nodes(relations: dict, model: str, max_entities: int, concurrency: int) -> int:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(future_to_node):
            ...
            try:
                ...
                print(f"[l2] {done}/{total} {kind} {label} ERROR {type(e).__name__}", flush=True)
                continue
...
def main() -> None:
    ...
    l2_updated = 0
    if not args.no_l2:
        l2_updated = l2_enrich_nodes(relations, args.model, args.max_entities, args.concurrency)
